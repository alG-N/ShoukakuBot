...
class AfkRepository {
     ...
     */
    async getAfk(userId, guildId = null) {
        try {
            // First check for global AFK
            const globalAfk = await postgres_js_1.default.query(`SELECT * FROM user_afk WHERE user_id = $1 AND guild_id IS NULL`, [userId]);
            ...
            if (guildId) {
                const guildAfk = await postgres_js_1.default.query(`SELECT * FROM user_afk WHERE user_id = $1 AND guild_id = $2`, [userId, guildId]);
    ...
    async setAfk(data) {
        try {
            ...
            if (type === 'global') {
                ...
                await postgres_js_1.default.query(`DELETE FROM user_afk WHERE user_id = $1 AND guild_id IS NOT NULL`, [userId]);
                // Then upsert the global AFK
                await postgres_js_1.default.query(`INSERT INTO user_afk (user_id, guild_id, reason, timestamp, type)
                     VALUES ($1, NULL, $2, $3, 'global')
                     ON CONFLICT (user_id, guild_id) 
                     DO UPDATE SET reason = $2, timestamp = $3, type = 'global', updated_at = NOW()`, [userId, reason, timestamp]);
            }
            else {
                // For guild AFK, first check if user has global AFK
                const hasGlobal = await postgres_js_1.default.query(`SELECT 1 FROM user_afk WHERE user_id = $1 AND guild_id IS NULL`, [userId]);
                // If user has global AFK, remove it first
                if (hasGlobal.rows.length > 0) {
                    await postgres_js_1.default.query(`DELETE FROM user_afk WHERE user_id = $1 AND guild_id IS NULL`, [userId]);
                }
                // Upsert guild-specific AFK
                await postgres_js_1.default.query(`INSERT INTO user_afk (user_id, guild_id, reason, timestamp, type)
                     VALUES ($1, $2, $3, $4, 'guild')
                     ON CONFLICT (user_id, guild_id) 
                     DO UPDATE SET reason = $3, timestamp = $4, type = 'guild', updated_at = NOW()`, [userId, guildId, reason, timestamp]);
            }
            return true;
        }
        catch (error) {
     ...
     */
    async removeAfk(userId, guildId = null) {
        try {
            ...
            if (currentAfk.type === 'global') {
                ...
                await postgres_js_1.default.query(`DELETE FROM user_afk WHERE user_id = $1 AND guild_id = $2`, [userId, guildId]);
            }
            return currentAfk;
        }
        catch (error) {
     ...
     */
    async isUserAfk(userId, guildId = null) {
        return this.getAfk(userId, guildId);
    }
    /**
     * Get all AFK users in a guild (for bulk operations)
     */
    async getGuildAfkUsers(guildId) {
        try {
            const result = await postgres_js_1.default.query(`SELECT * FROM user_afk WHERE guild_id = $1 OR guild_id IS NULL`, [guildId]);
            return result.rows;
        }
        catch (error) {
     ...
     */
    async getMultipleAfk(userIds, guildId) {
        try {
            if (userIds.length === 0)
                return new Map();
            const result = await postgres_js_1.default.query(`SELECT * FROM user_afk 
                 WHERE user_id = ANY($1) 
                 AND (guild_id = $2 OR guild_id IS NULL)`, [userIds, guildId]);
     ...
     */
    async cleanupOldAfk(maxAgeDays = 30) {
        try {
            const cutoffTimestamp = Date.now() - (maxAgeDays * 24 * 60 * 60 * 1000);
            const result = await postgres_js_1.default.query(`DELETE FROM user_afk WHERE timestamp < $1`, [cutoffTimestamp]);
...
exports.AfkRepository = AfkRepository;
// Export singleton instance
const afkRepository = new AfkRepository();
exports.default = afkRepository;
//# sourceMappingURL=AfkRepository.js.map